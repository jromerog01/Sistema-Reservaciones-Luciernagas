...
const initializeParkGalleryModal = () => {
    ...
    if (!galleryModal || !galleryButtons.length || !galleryModalImage) {
        return;
    }
    ...
    const renderGalleryImage = () => {
        ...
        galleryModalImage.classList.remove("is-zoomed");
    ...
    galleryModalImage?.addEventListener("click", (event) => {
        ...
        const isZoomed = galleryModalImage.classList.contains("is-zoomed");
        ...
        if (isZoomed) {
            ...
            galleryModalImage.classList.add("is-zoomed");
        }
    });

    document.addEventListener("click", (event) => {
        const button = event.target.closest && event.target.closest("[data-gallery-image]");

        if (!button) {
            return;
        }

        const index = Array.from(galleryButtons).indexOf(button);
        openGallery(Number.parseInt(button.dataset.galleryIndex || String(index), 10) || index);
    });
